...
export default function Verify({ auth }) {
    ...
    const handleVerify = async (e) => {
        ...
        try {
            const res = await axios.post(route("warranty.verify"), { imei });
            setResult(res.data);
        } catch (err) {
                            ...
                            {result.verified ? (
                                <div className="bg-green-50 border border-green-400 text-green-900 p-6 rounded-xl space-y-3 shadow-inner animate-fade-in">
                                    ...
                                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-[16px]">
                                        ...
                                        <li><strong>Activated On:</strong> {result.product.warranty_activated}</li>
                                        <li><strong>Warranty Period:</strong> {result.product.warranty_period_days} days</li>
                                        <li><strong>Expires On:</strong> {result.product.warranty_expires_at}</li>
                                        <li><strong>Days Remaining:</strong> <span className="font-semibold text-green-700">{result.product.warranty_remaining_days} days</span></li>
                                    </ul>
                                </div>
                            ) : (
                                <div className="bg-red-50 border border-red-400 text-red-800 p-6 rounded-xl text-lg flex items-center gap-3 shadow-inner animate-fade-in">
                                    ...
                                    Invalid IMEI or Warranty Not Activated.
                                </div>
                            )}
                        </div>
                    )}
